...
def collate_fn(batch):
    ...
    for item in batch:
        if len(item) == 4:  # PretrainVLDataset 返回 4 个值
            X, Y, loss_mask, image_tensors = item
            image_tensors_batch.append(image_tensors)
        else:  # 其他数据集返回 3 个值
            X, Y, loss_mask = item
    ...
    # 如果有图像张量，添加到结果中
    if image_tensors_batch:
        result['image_tensors'] = torch.stack(image_tensors_batch)
...
def train_one_epoch(model, train_loader, optimizer, scaler, epoch, wandb, args):
    ...
    for step, batch in enumerate(tqdm(train_loader, desc=f"Training of epoch {epoch}", total=iter_per_epoch)):
        if args.train_model == "llm-vl":
            X, Y, loss_mask, pixel_tensors = batch['X'], batch['Y'], batch['loss_mask'], batch["image_tensors"]
...
if __name__ == "__main__":
    ...
    llm_model_check_point_path = "/mnt/d/linux/LLM/MiniLLM/assets/minillm_output/sft/dim_512/minillm_sft_v2.0_build20250617.pth"
    ...
    parser.add_argument("--out_dir", type=str, help="The output directory",default=model_output_dir)
    ...
    parser.add_argument("--llm_checkpoint_path", type=str,default=llm_model_check_point_path)
    parser.add_argument("--vl_checkpoint_path", type=str, default=vl_model_check_point_path)
    ...
    if args.train_model == "llm":
        ...
        model, tokenizer, preprocess = init_vl_model(lm_config, args.tokenizer_path, args)
