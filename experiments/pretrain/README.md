...
- 测试更大的tokenizer vocab（qwen2）对收敛速度的影响，以及观测是否可以解决minimind测试时生成�符号问题
  1. 现象：loss更大，收敛速度更慢，测试验证效果更差。总体符合猜测预期。
  2. 并未再次出现�符号，但是由于训练时间较短，未进行有效收敛。
- 测试自己训练的tokenizer和qwen2、minimind的性能差距
  1. 我们发现自己训练tokenizer时设定的vocab为6400，但是加载后vocab就变为了6402，排查分析时由于add_bos_token=True,add_prefix_space=False,导致增加了两个。
   
- 重新训练一个更大的tokenizer vocab（32K）对收敛速度的影响，以及对最终模型效果的影响✅
  1. 现象：loss 下降速度并没有明显变化，但是验证模型的效果要好于minimind的tokenizer（6400 vocab size），且未出现�符号。
  2. 结论：根据实际情况调整tokenizer的vocab size，在效率和效果上进行平衡。更大的tokenizer vocab size 可以提高模型的效果，但是也会导致收敛速度变慢。（合理增加tokenizer 收益更大）。
  3. 第一轮训练的时候由于设置的epoch为1，导致动态学习率下降速度较快，从而模型loss下降速度很快。启动第二轮继续训练时，由于增大了epoch，从而动态学习率下降速度变慢，从而模型loss下降速度变慢。需要分析两者是否有关系。
     1. 测试方案：再次启动模型训练，降低学习率，降低epoch，观察loss下降速度是否变慢。
     2. 现象：差距并不明显，第一轮loss下降速度快是因为模型刚开始学习，速度快正常。
  
- 切换更大的数据集（wiki&baike）进行训练
  1. 现象：对数据集未进行长度控制（理论上是非必须的），但由于模型较小，且预训练阶段模型输入的长度被限制到了512，导致使用该数据集训练后模型很难生成eos_token。
  2. 解决方案：对原始训练数据的长度进行控制，保证每条数据都存在eos_token；继续训练观察。
  3. 现象：更换成新的数据集（最大长度为512）后训练一个epoch可以明显观测到模型可以根据内容生成eos_token，不会一直重复生成。

### 最终结果
- 训练出了一个“基本”能生成文字的模型，但是生成的和真实数据完全不相关，部分情况下会生成重复的内容，尤其是生成的后半段会一直重复（生成eos_token的概率较低），
- 决策：暂时将这一版模型保存，记作，继续进行下一步的sft微调训练。

- TODO 有一个脑洞大开的想法：新建一个模型使用和qwen2相同的tokenizer，然后在预训练阶段完全使用qwen模型生成过程中的tokenizer概率值进行计算分布（蒸馏）